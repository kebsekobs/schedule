...
function sortToExpand(data) {
  ...
  const firstYearBach = data.filter(
    (group) =>
      Math.floor(Number(group.groupId) / 100) === 1 &&
      group.magistracy === false
  );
  const secondYearBach = data.filter(
    (group) =>
      Math.floor(Number(group.groupId) / 100) === 2 &&
      group.magistracy === false
  );
  const thirdYearBach = data.filter(
    (group) =>
      Math.floor(Number(group.groupId) / 100) === 3 &&
      group.magistracy === false
  );
  const fourthYearBach = data.filter(
    (group) =>
      Math.floor(Number(group.groupId) / 100) === 4 &&
      group.magistracy === false
  );
  const fifthYearBach = data.filter(
    (group) =>
      Math.floor(Number(group.groupId) / 100) === 5 &&
      group.magistracy === false
  );
